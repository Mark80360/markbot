...
class PermissionApprover:
    ...
    async def __call__(
        ...
        if self._question_tool is not None and hasattr(
            ...
            # Register the session-keyed mapping so plain-text replies
            # (channels that don't round-trip question_id metadata) are
            # still routed to this approval by the loop's session lookup.
            if session_key and hasattr(self._question_tool, "register_pending"):
        ...
        try:
            ...
            if self._question_tool is not None:
                if hasattr(self._question_tool, "_pending_questions"):
                    self._question_tool._pending_questions.pop(approval_id, None)
                if session_key and hasattr(self._question_tool, "unregister_pending"):
...
def _parse_choice(response: str) -> str:
    ...
    Handles the full range of user inputs:
      - Bare numbers: ``"1"`` / ``"2"`` / ``"3"``
      - Labels: ``"Allow"`` / ``"Allow All"`` / ``"Deny"``
      - ``"User selected: …"`` prefix from AskUserQuestionTool
      - ``[Q:uuid]`` markers echoed by the user
      - Option lines like ``"1. Allow — run once"``
    """
    text = (response or "").strip().lower()
    # Handle "User selected: ..." prefix from AskUserQuestionTool.
    if "user selected:" in text:
        text = text.split("user selected:", 1)[1].strip()
    # Strip [Q:...] marker if the user echoed it.
    if "[q:" in text:
        text = text.split("[q:", 1)[0].strip()

    # allow_all — check first because "allow all" contains "allow".
    ...
    # allow / deny by bare number or short keyword.
    parts = text.split()
...
def _apply_choice(choice: str, approver: PermissionApprover, session_key: str) -> bool:
    """Apply a parsed choice: set allow_all flag if needed, return bool."""
    if choice == "allow_all":
...
async def _prompt_stdin(prompt: str) -> str:
    ...
    Returns the parsed choice string ("allow", "allow_all", or "deny").
    """
    import sys
