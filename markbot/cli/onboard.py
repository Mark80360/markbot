"""Interactive onboarding questionnaire for markbot."""
...
from markbot.cli.models import (
    format_token_count,
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from markbot.config.schema to get
    ...
    from markbot.config.schema import AgentDefaults
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"markbot.channels.{name}")
