...
class CronService:
    ...
    def _load_store(self) -> CronStore:
        ...
        if self.store_path.exists():
            try:
                ...
                logger.warning("Cron store load failed, starting with empty store")
    ...
    def _save_store(self) -> None:
        ...
        try:
            ...
            # Atomic rename ensures data integrity even if crash occurs during write
    ...
    async def _execute_job(self, job: CronJob) -> None:
        ...
        # Handle one-shot and delete-after-run jobs
        if job.delete_after_run:
            self._store.jobs = [j for j in self._store.jobs if j.id != job.id]
            logger.info("Cron: job '{}' deleted after run", job.name)
